...
def predictions(weather_turnstile):
    ...
    # helper functions
    #
    du=pandas.get_dummies(weather_turnstile['UNIT'],prefix='unit')
    hr=pandas.get_dummies(weather_turnstile['Hour'],prefix='hour')
    feat=weather_turnstile[['rain','mintempi']].join(du).join(hr)
    val=weather_turnstile[['ENTRIESn_hourly']]
    feat['ones']=np.ones(len(val))
    prediction=sm.OLS(val,feat).fit().predict(feat)
    return prediction
